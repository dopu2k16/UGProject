...
source('C:\\Users\\DIPU\\Desktop\\yelpTopicM\\explore\\igraph.plot2.R')

graph.circular <- function(restaurant.reviews.positive.topicmodel,k,n,topic.names=c()){
  
  topic.terms <- data.frame(terms(restaurant.reviews.positive.topicmodel,n),stringsAsFactors = FALSE)
  
  
  
  p <-lapply(seq(1,k),function(i) restaurant.reviews.positive.topicmodel@beta[i,match(topic.terms[,i],restaurant.reviews.positive.topicmodel@terms)])
